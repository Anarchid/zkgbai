...
import zkgbai.military.unitwrappers.Porc;
...
public class EconomyManager extends Module {
	...
	public Map<Integer ,Porc> dporcs = new HashMap<>();
	...
	List<Unit> newMexes = new ArrayList<>();
	List<Unit> newEnergy = new ArrayList<>();
	
	public Unit sparrow = null;
	...
	boolean hardEstalled = false;
	boolean estalled = false;
	...
	boolean bigMap;
	boolean waterDamage;
	public boolean defendedFac = false;
	...
	public int message(int player, String message) {
		if (ai.mergedAllies == 0 && message.startsWith("kgbfac")){
			...
			if (userFac != null){
				ai.say("Set first factory: " + callback.getUnitDefByName(userFac).getHumanName());
			}
	...
	public int update(int frame) {
		...
		if (frame % 30 == ai.offset % 30) {
			...
			for (Worker w:workers.values()){
				if (workers.size() > greed && (greed <= 2 || greed <= workers.size() * Math.min((1f - (graphManager.territoryFraction * graphManager.territoryFraction)), 1f))) break;
				if (w.isGreedy) {
					w.isGreedy = false;
					greed--;
				}
			...
			if (effectiveIncome > 30f && sparrow == null && !hasPlanes) morphSparrow();
	...
	public int unitCreated(Unit unit,  Unit builder) {
		...
		if (defName.equals("energysolar") || defName.equals("energywind")){
			newEnergy.add(unit);
		}
		...
		if(defName.equals("turretmissile") || defName.equals("turretlaser") || defName.equals("turretheavylaser") || defName.equals("turretgauss") || defName.equals("turretemp") || defName.equals("turretriot") || defName.equals("turretaalaser")){
			porcs.add(unit);
			if (!defName.equals("turretaalaser")) dporcs.put(unit.getUnitId(), new Porc(unit));
		}
	...
	public int unitDamaged(Unit h, Unit attacker, float damage, AIFloat3 dir, WeaponDef weaponDef, boolean paralyzed){
		...
		if (h.getUnitId() != lastRepairedUnit && h.getHealth() > 0 && !h.getDef().getName().contains("bomber")){
	...
	WorkerTask getCheapestJob( Worker worker){
		...
		if (worker.getTask() != null){
			...
			if (task instanceof ConstructionTask){
				ConstructionTask ctask = (ConstructionTask) task;
				if (ctask.target != null) cost -= 1000f * (ctask.target.getBuildProgress() * ctask.target.getBuildProgress());
			}
	...
	float costOfJob(Worker worker,  WorkerTask task){
		...
		for (Worker w: task.assignedWorkers){
			...
			if (w.id != worker.id){
				if (w.isCom){
					costMod += 2;
				}else if (rdist < dist || deltadist < 100f){
					costMod++;
				}
			}
		}
		...
		if (task instanceof ConstructionTask){
			...
			if (ctask.buildType.getUnitDefId() == buildIDs.solarID || ctask.buildType.getUnitDefId() == buildIDs.windID){
				...
				if (estalled || effectiveIncomeMetal > effectiveIncomeEnergy - 2.5f) {
					...
					if (worker.isCom){
						if (hardEstalled) return ((dist / 2f) - 300f) + Math.max(0, (1000 * (costMod - 2)));
						return ((dist / 3f) - 200f) + Math.max(0, (1000 * (costMod - 2)));
					}
					return ((dist / 3f) - 200f) + Math.max(0, (1000 * (costMod - 1)));
				}else {
					if (worker.isGreedy) return dist + (10000f * costMod); // greedy workers shouldn't build energy at all.
					if (worker.isCom) return ((dist/4f) + Math.max(0, (1000 * (costMod - 2))));
			...
			if (worker.isGreedy && !facManager.factories.isEmpty() && ((ctask.facDef && effectiveIncome < 15f) || (ctask.buildType.getCost(m) > 300f && ctask.buildType.getUnitDefId() != buildIDs.airpadID))){
			...
			if (ctask.facDef && worker.isCom){
				...
				if (f != null && f.getUnit().getHealth() < f.getUnit().getMaxHealth()){
					return -1000f;
				}
			}
			...
			if (buildIDs.expensiveIDs.contains(ctask.buildType.getUnitDefId())){
				// for expensive porc.
				if (hardEstalled) return dist + (250f * costMod);
				if (costMod < 4){
			...
			if (ctask.buildType.isAbleToAttack()){
				...
				if (worker.isCom && !estalled) return ((dist/4f) - 100f) + Math.max(0, (600f * (costMod - 2)));
				if (worker.isGreedy) return ((dist/3f) - 200f) + Math.max(0, (600f * (costMod - 1)));
				return ((dist/2f) - 200f) + Math.max(0, (1500 * (costMod - 2)));
			}
			
			if (ctask.buildType.getUnitDefId() == buildIDs.airpadID){
				// for airpads
				return (dist/3f) - 350f + Math.max(0, (500 * (costMod - 2)));
			}
			...
			if (ctask.buildType.getUnitDefId() == buildIDs.pylonID) {
				...
				return (dist/4f) - 100f + (500f * (costMod - 1));
			}
    ...
    void generateTasks(Worker worker){
		...
		//do we need storages?
		if (frame > 600 && !facManager.factories.isEmpty() &&
			  (metal/maxStorage > 0.8f || commanders.size() + storages.size() < 1 + ai.mergedAllies) && storageTasks.isEmpty()){
		...
		if (!worker.isGreedy && !facManager.factories.isEmpty() &&
			  havePlanes && airpadTasks.isEmpty() && staticIncome > 15f
				&& (airpads.size() < Math.round(warManager.bomberHandler.getBomberSize()/8f) || airpads.isEmpty())){
	...
	void createFactoryTask(Worker worker, boolean isPlop){
		...
		/*if (facManager.factories.size() == 0){
			...
			int i = (int) Math.ceil(Math.random() * (potentialFacList.size())) - 1;
			facName = potentialFacList.get(i);
			factory = callback.getUnitDefByName(facName);
		...
		if (facManager.factories.isEmpty() || isPlop) {
			while (!good) {
				...
				while (!callback.getFriendlyUnitsIn(position, clearance).isEmpty() || !callback.getFeaturesIn(position, clearance).isEmpty() || distance(graphManager.getClosestSpot(position).getPos(), position) < 100 || !buildCheck(new ConstructionTask(factory, position, 0))) {
					if (j++ > 5) break;
					position = (i < 8) ? getAngularPoint(worker.getPos(), graphManager.getMapCenter(), clearance + 20f) : getRadialPoint(worker.getPos(), 125f);
					position = callback.getMap().findClosestBuildSite(factory,position,600f, 3, facing);
				}
	...
	void fortifyMex(AIFloat3 position, WeaponDef wd){
		UnitDef porc;
		ConstructionTask ct;
		AIFloat3 pos;
	...
	void defendNano(Nanotower n){
		if (n.target == null || n.target.getHealth() <= 0 || n.target.getTeam() != ai.teamID) return;
		UnitDef llt = callback.getUnitDefByName("turretlaser");
		AIFloat3 pos;
		
		pos = getDirectionalPoint(n.target.getPos(), n.unit.getPos(), distance(n.target.getPos(), n.unit.getPos()) + 100f);
		pos = callback.getMap().findClosestBuildSite(llt, pos, 600f, 3, 0);
